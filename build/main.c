...
#include "../GUI/output/output_wnd.h"
					...
					case MENU_START_ID:
						{
							...
							plt_sys_t plt_sys;
							plt_sys_create(&plt_sys,total);
							MPFR_DECL_INIT(esc_dis,MPFR_DEFAULT_PRECISION);
							MPFR_DECL_INIT(esc_time,MPFR_DEFAULT_PRECISION);

							plt_sys_entity_t * arr = plt_sys_get_ent(&plt_sys,0);

							input_quantity_get_num(&RealDis_To_ScreenPix,esc_dis);
							input_quantity_get_num(&RealTime_To_ScreenTime,esc_time);
							...
							quantity_errno = true;
								...
								)
								{
									...
									plt_sys.len = i;
									plt_sys_destroy(&plt_sys);
									arr = NULL;
									mpfr_clears(esc_dis,esc_time,(mpfr_ptr)0);
									break;
								}
                                                        }
							if(arr == NULL)
							...
							output_start(plt_sys,esc_time,esc_dis);
						}
						break;
					default:
						quantity_errno = true;
