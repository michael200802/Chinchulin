...
void* render_routine(void*_arg)
{
	...
	for(;;)
	{
		...
		for(size_t i = 0; i < render->pu_arg->len; i++)
		{
			...
			mpfr_div(x_pos,x_pos,render->pu_arg->esc_dis,MPFR_RNDN);
			mpfr_div(y_pos,y_pos,render->pu_arg->esc_dis,MPFR_RNDN);
			input_img_t img = {
					.screen_x = mpfr_get_si(x_pos,MPFR_RNDN),
					.screen_y = -1*mpfr_get_si(y_pos,MPFR_RNDN),
				};


			//set radius
			MPFR_DECL_INIT(radius,MPFR_DEFAULT_PRECISION);
			mpfr_div(radius,ent->body.radius,render->pu_arg->esc_dis,MPFR_RNDN);
			img.radius = mpfr_get_ui(radius,MPFR_RNDN)+1;
			...
			if(ent->coord.origin)
			{
				...
				mpfr_div(x_pos,x_pos,render->pu_arg->esc_dis,MPFR_RNDN);
				mpfr_div(y_pos,y_pos,render->pu_arg->esc_dis,MPFR_RNDN);
				img.lend_x = mpfr_get_si(x_pos,MPFR_RNDN);
				img.lend_y = -1*mpfr_get_si(y_pos,MPFR_RNDN);
			}
			else
			{
				img.lend_x = 0;
			...
			//color of the circle
			float cur_angle = mpfr_get_flt(ent->coord.cur_angle,MPFR_RNDN);
...
plt_sys_entity_t* __get_entities_arr(input_planet_sys_child_t* plt_sys, plt_sys_entity_t* arr)
{
	...
	MPFR_DECL_INIT(big_mass,MPFR_DEFAULT_PRECISION);mpfr_set(big_mass,plt_sys_ent->body.mass,MPFR_RNDN);
        for(size_t i = 0; i < plt_sys->planet_sys_childs_arr_len; i++)
        {
